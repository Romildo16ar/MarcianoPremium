...
open class Marciano {
    ...
    open fun responda(frase: String, vararg extras: Any): String {
        ...
        return when {
            ...
            ehGrito(fraseAjustada) && ehPergunta(fraseAjustada)
    ...
    protected fun ehSilencio(frase: String): Boolean = frase.isBlank()
    ...
    protected fun ehPergunta(frase: String): Boolean = frase.trimEnd().endsWith("?")


    protected fun contemEu(frase: String): Boolean {
        return Regex("\\beu\\b", RegexOption.IGNORE_CASE).containsMatchIn(frase)
    }
}
